...
class DropBoxTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
    ...
    if (item.isConnectionTested) {
      try {
        const body = {
          ...
          typeOf: item.typeOf,
          folderName: item.fields.length
            ? item.fields.find((node) => node.key === "directory")
              ? item.fields.find((node) => node.key === "directory").value
              : ""
            : "",
        };
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      ...
      try {
        httpClient
          .post(DROPBOX_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your DropBox account
