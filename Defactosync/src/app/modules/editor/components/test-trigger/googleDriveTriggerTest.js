...
class GoogleDriveTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      const body = {
        ...
        commonInfo: item.commonInfo,
        nodeId: item.id,
        googleDriveFolderId: item.fields.length
        ? item.fields.find((node) => node.key === "folder")
          ? item.fields.find((node) => node.key === "folder").value
          : ""
        : ""
      };
      this.props.loaderOnSelectEvent(true);
      try {
        httpClient
          .post(GOOGLEDRIVE_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {             
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your GoogleDrive account
