...
import { showErrorToaster, queryStringToJson } from "appUtility/commonFunction";
import {
  hideMessage,
  showAuthLoader,
  onSelectEcho,
...
class MailChimpTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
        ...
        httpClient
          ...
          .then((res) => {
            if (res.data.statusCode === 200) {
              let jsonData = queryStringToJson(res.data.data.responseInfo);
              this.setState({
                triggerTestInfo: jsonData,
                isTested: true,
              });
            } else {
              this.setState({
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      const body = {
        ...
        commonInfo: item.commonInfo,
        nodeId: item.id,
        mailChimpListId: item.fields.length
          ? item.fields.find((node) => node.key === "list_id")
            ? item.fields.find((node) => node.key === "list_id").value
            : ""
          : "",
      };
      this.props.loaderOnSelectEvent(true);
      try {
        httpClient
          .post(MAILCHIMP_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your MailChimp account
