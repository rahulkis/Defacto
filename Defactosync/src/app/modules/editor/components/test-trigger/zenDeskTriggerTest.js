...
class ZenDeskTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
    ...
    if (item.isConnectionTested) {
      try {
        const body = {
          ...
          nodeId: item.id,
          commonInfo: item.commonInfo.toString(),
          typeOf: item.typeOf,
        };
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      const body = {
        ...
        commonInfo: item.commonInfo.toString(),
        nodeId: item.id,
        zendeskTicketId: item.fields.length
          ? item.fields.find((node) => node.key === "ticket")
            ? item.fields.find((node) => node.key === "ticket").value.toString()
            : ""
          : "",
      };
      this.props.loaderOnSelectEvent(true);
      ...
      try {
        httpClient
          .post(ZENDESK_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your ZenDesk account
