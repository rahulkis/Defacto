...
class HelpScoutTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
    ...
    if (item.isConnectionTested) {
      try {
        const body = {
          ...
          typeOf: item.typeOf,
          mailBoxId: item.fields.length
            ? item.fields.find((node) => node.key === "mailbox")
              ? item.fields.find((node) => node.key === "mailbox").value
              : ""
            : "",
        };
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      ...
      try {
        httpClient
          .post(HELPSCOUT_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your Helpscout account
