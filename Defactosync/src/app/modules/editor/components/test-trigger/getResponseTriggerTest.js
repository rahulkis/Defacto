...
class GetResponseTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
        ...
        httpClient
          ...
          .then((res) => {         
            if (res.data.statusCode === 200) {
              this.setState({
                triggerTestInfo: JSON.parse(res.data.data.responseInfo),
                isTested: true,
              });
            } else {
              this.setState({
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      ...
      try {
        httpClient
          .post(GETRESPONSE_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your GetResponse account
