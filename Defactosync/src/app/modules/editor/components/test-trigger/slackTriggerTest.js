...
class SlackTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
    ...
    if (item.isConnectionTested) {
      try {
        const body = {
          ...
          nodeId: item.id,
          channelId: item.fields.find((x) => x.key === "channel")
            ? item.fields.find((x) => x.key === "channel").value
            : "",
          memberId: item.meta.memberId,
        };      
        this.props.loaderOnSelectEvent(true);
        httpClient
          .post(SLACK_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      const body = {
        ...
        memberId: item.meta.memberId,
        nodeId: item.id,
        channelId: item.fields.length
          ? item.fields.find((node) => node.key === "channel")
            ? item.fields.find((node) => node.key === "channel").value
            : ""
          : "",
      };
      ...
      try {
        httpClient
          .post(SLACK_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your Slack account
