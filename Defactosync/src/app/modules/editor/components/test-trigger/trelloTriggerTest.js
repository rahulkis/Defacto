...
class TrelloTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
    ...
    if (item.isConnectionTested) {
      try {
        const body = {
          ...
          typeOf: item.typeOf,
          boardId: item.fields.length
            ? item.fields.find((node) => node.key === "board")
              ? item.fields.find((node) => node.key === "board").value
              : ""
            : "",
          listId: item.fields.length
            ? item.fields.find((node) => node.key === "list")
              ? item.fields.find((node) => node.key === "list").value
              : ""
            : "",
          cardId: item.fields.length
            ? item.fields.find((node) => node.key === "card")
              ? item.fields.find((node) => node.key === "card").value
              : ""
            : "",
          labelId: item.fields.length
            ? item.fields.find((node) => node.key === "label_id")
              ? item.fields.find((node) => node.key === "label_id").value
              : ""
            : "",
        };
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      ...
      try {
        httpClient
          .post(TRELLO_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your Trello account
