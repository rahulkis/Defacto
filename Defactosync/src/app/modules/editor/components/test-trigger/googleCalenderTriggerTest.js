...
class GoogleCalenderTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  handleTestTrigger = () => {
    const item = this.props.selectedNode;
    let time = item.fields.length
      ? item.fields.find((node) => node.key === "time_length")
        ? item.fields.find((node) => node.key === "time_length").value
        : "15"
      : "15";
    let timeUnit = item.fields.length
      ? item.fields.find((node) => node.key === "time_unit")
        ? item.fields.find((node) => node.key === "time_unit").value
        : "minutes"
      : "minutes";
    let googleCalendarStartTime = "";
    let googleCalendarEndTime = "";
    if (time && timeUnit) {
      if (item.selectedEvent.value === "event_start" && item.selectedEvent.value === "event_end") {
        googleCalendarStartTime = moment()
          .subtract(time, timeUnit).format();
          googleCalendarEndTime = moment().format(); 
       }
       // else {
      //   googleCalendarEndTime = moment().format();
      // }
    ...
    if (item) {
      const body = {
        ...
        commonInfo: item.commonInfo,
        nodeId: item.id,
        googleCalendarId: item.fields.length
          ? item.fields.find((node) => node.key === "calendarid")
            ? item.fields.find((node) => node.key === "calendarid").value
            : ""
          : "",
        googleCalendarStartTime: googleCalendarStartTime,
        googleCalendarEndTime: "",
        searchTerm: item.fields.length
        ? item.fields.find((node) => node.key === "search_term")
          ? item.fields.find((node) => node.key === "search_term").value
          : ""
        : ""
      };  
      ...
      try {
        httpClient
          .post(GOOGLE_CALENDER_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your GoogleCalendar account
