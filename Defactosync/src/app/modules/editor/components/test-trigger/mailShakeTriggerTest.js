...
class MailShakeTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      const body = {
        ...
        commonInfo: item.commonInfo,
        nodeId: item.id,
        mailshakeCampaignId: item.fields.length
          ? item.fields.find((node) => node.key === "campaignID")
            ? item.fields.find((node) => node.key === "campaignID").value
            : ""
          : "",
        mailshakeAssignedId: item.fields.length
          ? item.fields.find((node) => node.key === "assignedToEmailAddress")
            ? item.fields.find((node) => node.key === "assignedToEmailAddress")
                .value
            : ""
          : "",
        mailshakeLeadStatus: item.fields.length
          ? item.fields.find((node) => node.key === "status")
            ? item.fields.find((node) => node.key === "status").value
            : ""
          : "",
        mailshakeLinkUrl: item.fields.length
          ? item.fields.find((node) => node.key === "matchUrl")
            ? item.fields.find((node) => node.key === "matchUrl").value
            : ""
          : "",
        mailshakeIsDuplicate: "",
        mailshakeMessageType: item.fields.length
          ? item.fields.find(
              (node) =>
                node.key === "campaignMessageType" || node.key === "replyType"
            )
            ? item.fields.find(
                (node) =>
                  node.key === "campaignMessageType" || node.key === "replyType"
      ...
      try {
        httpClient
          .post(MAILSHAKE_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your MailShake account
