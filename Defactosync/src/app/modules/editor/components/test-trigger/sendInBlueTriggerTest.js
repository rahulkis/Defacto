...
class SendInBlueTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      const body = {
        ...
        cliType: item.selectedCLI,
        commonInfo: item.commonInfo,       
        listIds: item.fields.length
        ? item.fields.find((node) => node.key === "listId")
          ? item.fields.find((node) => node.key === "listId").value.toString()
          : ""
        : "",
        nodeId: item.id
      };
      ...
      try {
        httpClient
          .post(SENDINBLUE_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your SendInBlue account
