...
class MailerliteTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      const body = {
        ...
        commonInfo: item.commonInfo,
        nodeId: item.id,
        webFormId: item.fields.length
          ? item.fields.find((node) => node.key === "webform_id")
            ? item.fields.find((node) => node.key === "webform_id").value
            : ""
          : "",
          groupId: item.fields.length
          ? item.fields.find((node) => node.key === "group_id")
            ? item.fields.find((node) => node.key === "group_id").value
            : ""
          : "",
      ...
      try {
        httpClient
          .post(MAILERLITE_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your Mailerlite account
