...
class IntercomTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
      const body = {
        ...
        commonInfo: item.commonInfo,
        nodeId: item.id,
        tagId: item.fields.length
        ? item.fields.find((node) => node.key === "tag_id")
          ? item.fields.find((node) => node.key === "tag_id").value
          : ""
        : "",       
      };
      this.props.loaderOnSelectEvent(true);
      try {
        httpClient
          .post(INTERCOM_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {           
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your Intercom account
