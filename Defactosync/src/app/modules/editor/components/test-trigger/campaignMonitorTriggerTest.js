...
class CampaignMonitorTriggerTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
    ...
    if (item.isConnectionTested) {
      try {
        const body = {
          ...
          typeOf: item.typeOf,
          campaignMonitorClientId: item.fields.length
          ? item.fields.find((node) => node.key === "ClientID")
            ? item.fields.find((node) => node.key === "ClientID").value
            : ""
          : "",
          campaignMonitorListId: item.fields.length
          ? item.fields.find((node) => node.key === "ListID")
            ? item.fields.find((node) => node.key === "ListID").value
            : ""
          : ""
        };   
  ...
  handleTestTrigger = () => {
    ...
    if (item) {
     ...
     this.props.loaderOnSelectEvent(true);      
      try {
        httpClient
          .post(CAMPAIGN_MONITOR_WEBHOOK_URLS.GET_RESPONSE, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
        ...
        {!nodeItem.isConnectionTested && !isTested && !isTestInfo && (
          <div className="text-center">
            ...
            <div className="mt-2 info-text">
              <p>
                We'll find a recent message posted in your CampaignMonitor account
