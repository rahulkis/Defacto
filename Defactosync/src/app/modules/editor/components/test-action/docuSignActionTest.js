...
import {
  APP_IMAGE_URL,
  IMAGE_FOLDER,
  DOCUSIGN_WEBHOOK_URLS
} from "constants/AppConst";

class DocuSignActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(DOCUSIGN_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    ...
    if (
      ...
      if (item.selectedNode.selectedEventName === "create_signature_request") {
        apiUrl = apiUrl.replace("{accountId}", item.selectedNode.commonInfo);
        bodyInfo = {
          templateId: bodyInfo.templateid,
          templateRoles: [
            {
              email: bodyInfo.emailblurb || "",
              name: bodyInfo.recipientname || "",
              roleName: bodyInfo.recipientrole || "",
          },
        ],
        status: "sent"
        };
      ...
      try {
        httpClient
          .post(DOCUSIGN_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
