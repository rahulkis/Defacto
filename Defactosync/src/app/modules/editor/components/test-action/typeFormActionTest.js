...
import {
    APP_IMAGE_URL,
    IMAGE_FOLDER,
    TYPEFORM_WEBHOOK_URLS
} from "constants/AppConst";

class TypeFormActionTest extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            anchorEl: undefined,
    ...
    componentDidMount() {
        ...
        if (item.selectedNode.isConnectionTested) {
            try {
                ...
                httpClient
                    .post(TYPEFORM_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
                    .then((res) => {
    ...
    handleTestAction = () => {
        ...
        if (
            ...
            if (item.selectedNode.selectedEventName === "create_empty_form") {

                bodyInfo.workspace = bodyInfo.workspace ?
                    bodyInfo = {
                        "title": bodyInfo.title,
                        "workspace": {
                            "href": `https://api.typeform.com/workspaces/${bodyInfo.workspace}`
                        }
            ...
            if (item.selectedNode.selectedEventName === "lookup_responses") {
                apiUrl = apiUrl.replace("{form_id}", bodyInfo.form).replace("{completed}", bodyInfo.complete);
                if (bodyInfo.since) {
                    apiUrl = `${apiUrl}&since=${bodyInfo.since}`
                }
                if (bodyInfo.until) {
                    apiUrl = `${apiUrl}&until=${bodyInfo.until}`
                }
                if (bodyInfo.search) {
                    apiUrl = `${apiUrl}&query=${bodyInfo.search}`
                }
                if (bodyInfo.complete) {
                    apiUrl = `${apiUrl}&completed=${bodyInfo.complete === "true" ? true : false}`
                }
                apiUrl = apiUrl.replace("{page_size}", Number(bodyInfo.fetch) || 25);
            }
            ...
            try {
                httpClient
                    .post(TYPEFORM_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
                    .then((res) => {
