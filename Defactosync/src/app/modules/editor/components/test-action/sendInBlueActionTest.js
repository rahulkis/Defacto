...
class SendInBlueActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(SENDINBLUE_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    ...
    if (
      ...
      if (item.selectedNode.selectedEventName === "add_update_contact") {
        bodyInfo = {
          email: bodyInfo.email,
          attributes: {
            LASTNAME: bodyInfo.lname || "",
            FIRSTNAME: bodyInfo.fname || "",
          },
          updateEnabled: true,
          listIds: [bodyInfo.listIds],
        };
      ...
      const body = {
        headerValue: {
          "api-key": `${item.connectionData.token}`,
      ...
      try {
        httpClient
          .post(SENDINBLUE_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
