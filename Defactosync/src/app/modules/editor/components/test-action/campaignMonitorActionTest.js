...
class CampaignMonitorActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(CAMPAIGN_MONITOR_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
              if (item.selectedNode.selectedEventName === "add_subscriber") {
                let bodyInfo = {
                  email: res.data.data.responseInfo.body,
                };
                res.data.data.responseInfo.body = bodyInfo;
              }
  ...
  handleTestAction = () => {
    ...
    if (
      ...
      if (item.selectedNode.selectedEventName === "unsubscribe") {
        apiUrl = apiUrl.replace("{listid}", bodyInfo.ListID);
        bodyInfo = {
      ...
      if (
        item.selectedNode.selectedEventName === "add_subscriber" ||
        item.selectedNode.selectedEventName === "update_subscriber"
      ) {
        if (item.selectedNode.selectedEventName === "update_subscriber") {
          apiUrl = apiUrl
            .replace("{listid}", bodyInfo.ListID)
        ...
        bodyInfo = {
          ...
          ConsentToTrack: "No",
          Resubscribe: bodyInfo.Resubscribe ? bodyInfo.Resubscribe : false,
          RestartSubscriptionBasedAutoresponders: bodyInfo.RestartSubscriptionBasedAutoresponders
            ? bodyInfo.RestartSubscriptionBasedAutoresponders
            : false,
        };
      }
      ...
      try {
        httpClient
          .post(CAMPAIGN_MONITOR_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {     
            if (res.data.statusCode === 200) {
              ...
              if (!res.data.responseBody.errorMessage) {
                if (item.selectedNode.selectedEventName === "add_subscriber") {
                  let bodyInfo = {
                    email: res.data.responseBody,
                  };
                  res.data.responseBody = bodyInfo;
                }
                this.setState({
