...
import {
  APP_IMAGE_URL,
  IMAGE_FOLDER,
  STORYCHIEF_WEBHOOK_URLS
} from "constants/AppConst";


class StoryChiefActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(STORYCHIEF_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    const item = this.props;
      if (
        ...
        if (item.selectedNode.selectedEventName === "create_story") {
          apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl;
          let slug = bodyInfo.title.split(" ").length > 0 ? bodyInfo.title.toLowerCase().split(" ").join("-") : bodyInfo.title.toLowerCase();
          bodyInfo = {
               title : bodyInfo.title,
               content: bodyInfo.body,
               excerpt: bodyInfo.excerpt,
               featured_image: bodyInfo.featured_image,
               language: bodyInfo.language,
               seo_title: bodyInfo.title,
               seo_description: bodyInfo.excerpt,
               slug: slug
          };
          cleanObject(bodyInfo);
        };
  
        if (item.selectedNode.selectedEventName === "create_author") {
            apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl;
            bodyInfo = {
        ...
        if (item.selectedNode.selectedEventName === "create_category") {
          apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl;
          bodyInfo = {
                name: bodyInfo.name,
          };
        ...
        try {
          httpClient
            .post(STORYCHIEF_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
            .then((res) => {
              if (res.data.statusCode === 200) {
