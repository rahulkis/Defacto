...
class NotionActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(NOTION_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    ...
    console.log(item, "testAction");
      if (
        ...
        if (item.selectedNode.selectedEventName === "create_page") {
          apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl;
          bodyInfo = {
              parent: {
                  type: "page_id",
                  "page_id": bodyInfo.parent_page,
              },
              properties: {
                  title: [
                      {
                          text: {
                              content: bodyInfo.title
                          }
                      }
                  ]
              },
        ...
        if (item.selectedNode.selectedEventName === "create_database_item") {
            apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl;
            bodyInfo = {
                 parent: {
                   database_id: bodyInfo.database
                 },
                 properties: {
                   Name: {
                     title: [
        ...
        if (item.selectedNode.selectedEventName === "update_database_item") {
          apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl + bodyInfo.item;
          bodyInfo = {
            parent: {
              database_id: bodyInfo.database
            },
        ...
        try {
          httpClient
            .post(NOTION_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
            .then((res) => {
              if (res.data.statusCode === 200) {
