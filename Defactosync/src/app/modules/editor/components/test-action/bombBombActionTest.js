...
import {
  APP_IMAGE_URL,
  IMAGE_FOLDER,
  BOMBBOMB_WEBHOOK_URLS
} from "constants/AppConst";


class BombBombActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(BOMBBOMB_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    ...
    if (
      ...
      if (item.selectedNode.selectedEventName === "new_email_contact") {
        apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl;
        bodyInfo = {
          emailId: bodyInfo.email_title,
          emailAddress: bodyInfo.email_address
        };
        cleanObject(bodyInfo);
      };
      ...
      try {
        httpClient
          .post(BOMBBOMB_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
