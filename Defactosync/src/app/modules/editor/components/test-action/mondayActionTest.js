...
class MondayActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(MONDAY_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    const item = this.props;
      if (
        ...
        if (item.selectedNode.selectedEventName === "create_item") {
          bodyInfo = `mutation { create_item (board_id: ${bodyInfo.board_id}, item_name: "${bodyInfo.item_name}", group_id: "${bodyInfo.group_id}" ){id }}`
        };
  
        if (item.selectedNode.selectedEventName === "delete_group") {
            bodyInfo = `mutation { delete_group (board_id: ${bodyInfo.board_id}, group_id: "${bodyInfo.group_id}") {id deleted }}`
        };
  
        if (item.selectedNode.selectedEventName === "duplicate_board") {
            bodyInfo = `mutation { duplicate_board(board_id: ${bodyInfo.board_id}, duplicate_type: ${bodyInfo.duplicate_type}) { board { id }}}`
        };
  
        if (item.selectedNode.selectedEventName === "archive_board") {
          bodyInfo = `mutation { archive_board (board_id: ${bodyInfo.board_id}) { id }}`
        };
  
        if (item.selectedNode.selectedEventName === "create_board") {
          bodyInfo = `mutation { create_board (board_name: \"${bodyInfo.board_name}\", board_kind: ${bodyInfo.board_kind}) { id }}`
        };
  
        if (item.selectedNode.selectedEventName === "group_position") {
          bodyInfo = `mutation { duplicate_group (board_id: ${bodyInfo.board_id}, group_id: \"${bodyInfo.group_id}\", group_title: "${bodyInfo.new_group_title}", add_to_top: ${bodyInfo.should_add_group_to_top}) { id } }`
        };

        if (item.selectedNode.selectedEventName === "create_group") {
          bodyInfo = `mutation { create_group (board_id: ${bodyInfo.board_id}, group_name: \"${bodyInfo.group_name}\") { id } }`
        };

        if (item.selectedNode.selectedEventName === "create_column") {
          bodyInfo = `mutation { create_column (board_id: ${bodyInfo.board_id}, title:\"${bodyInfo.column_title}\", column_type: ${bodyInfo.column_type}) {id}}`
        };

        if (item.selectedNode.selectedEventName === "create_workspace") {
          bodyInfo = `mutation { create_workspace (name: \"${bodyInfo.name}\", kind: open, description: \"${bodyInfo.description}\") { id description } }`
        };
  
        const body = {
          headerValue: {  
            "Authorization": item.connectionData.token,
        ...
        try {
          httpClient
            .post(MONDAY_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
            .then((res) => {
              if (res.data.statusCode === 200) {
