...
import {
  APP_IMAGE_URL,
  IMAGE_FOLDER,
  GOOGLECONTACT_WEBHOOK_URLS
} from "constants/AppConst";


class GoogleContactActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(GOOGLECONTACT_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
    ...
    const item = this.props;
      if (
        ...
        if (item.selectedNode.selectedEventName === "create_contact") {
          ...
          let splitBirthday = bodyInfo.birthday && bodyInfo.birthday.split("/");
          let daybirthday = splitBirthday && splitBirthday[0];
          let monthbirthday = splitBirthday && splitBirthday[1];
          let yearbirthday = splitBirthday && splitBirthday[2];
            
          let splitEvent = bodyInfo.event_date && bodyInfo.event_date.split("/");
          let dayevent = splitEvent && splitEvent[0];
          let monthevent = splitEvent && splitEvent[1];
          let yearevent = splitEvent && splitEvent[2];

          bodyInfo = {
            names: [
                {
                    givenName: bodyInfo.first_name,
                    middleName: bodyInfo.middle_name,
                    familyName: bodyInfo.last_name,
                    honorificPrefix: bodyInfo.name_prefix,
                    honorificSuffix: bodyInfo.name_suffix,
                }
            ],
            organizations: [
                {
                    title: bodyInfo.job_title,
                    name: bodyInfo.company,
                    type: "work"
                }
            ],
            emailAddresses: [
                {
                    ...
                    type: bodyInfo.email_type
                }
            ],
            phoneNumbers: [
                {
                    value: bodyInfo.phone_number,
                    type: bodyInfo.phone_type
                }
            ],
            addresses: [
                {
                    streetAddress: bodyInfo.address_street,
                    poBox: bodyInfo.address_po_box,
                    extendedAddress: bodyInfo.address_neighborhood,
                    city: bodyInfo.address_city,
                    region: bodyInfo.address_state,
                    postalCode: bodyInfo.address_zip,
                    country: bodyInfo.address_country,
                    type: bodyInfo.address_type
                }
            ],
            birthdays: [
                {
                    date: {
                        day: Number(daybirthday),
                        month: Number(monthbirthday),
                        year: Number(yearbirthday)
                    }
                }
            ],
            events: [
                {
                    type: bodyInfo.event_type,
                    date: {
                        day: Number(dayevent),
                        month: Number(monthevent),
                        year: Number(yearevent)
                    }
                }
            ],
            urls: [
                {
                    value: bodyInfo.url,
                    type: bodyInfo.url_type
                }
            ],
            relations: [
                {
                    person: bodyInfo.related_person,
                    type: bodyInfo.realtionship_type
                }
            ]
          };
        };

        if (item.selectedNode.selectedEventName === "upload_contact_photo") {
            apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{contact_id}", bodyInfo.contact);
            const res = await this.getBase64FromUrl(bodyInfo.photo);
            if(res) {
                bodyInfo.photo = res.split(",")[1]
            }
            bodyInfo = {
                photoBytes: bodyInfo.photo,
                sources: [
        ...
        try {
          httpClient
            .post(GOOGLECONTACT_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
            .then((res) => {
              if (res.data.statusCode === 200) {
