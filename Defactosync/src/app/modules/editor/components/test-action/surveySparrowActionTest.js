...
import {
  APP_IMAGE_URL,
  IMAGE_FOLDER,
  SURVEYSPARROW_WEBHOOK_URLS
} from "constants/AppConst";


class SurveySparrowActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(SURVEYSPARROW_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
              let response = item.selectedNode.selectedEventName === "share_survey_email" ? {success: true} : res.data.data.responseInfo;
              this.setState({
  ...
  handleTestAction = () => {
    ...
    console.log(item, "testAction");
      if (
        ...
        if (item.selectedNode.selectedEventName === "share_survey_email") {
            apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{share_id}", bodyInfo.email_share_template);
            bodyInfo = {
              contacts: [ bodyInfo.recipient_email ]
            };
            cleanObject(bodyInfo);
        };
  
        if (item.selectedNode.selectedEventName === "share_nps_survey_email") {
          apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{survey_id}", bodyInfo.nps_survey);
          bodyInfo = {
            contacts: [
              { email: bodyInfo.recipient_email }
            ],
            name: bodyInfo.email_share_templates
          };
          cleanObject(bodyInfo);
        };
        ...
        try {
          httpClient
            .post(SURVEYSPARROW_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
            .then((res) => {
              if (res.data.statusCode === 200) {
                if (!res.data.responseBody.errorMessage) {
                  let response = item.selectedNode.selectedEventName === "share_survey_email" ? {success: true} : res.data.responseBody;
                  this.setState({
                    actionTestInfo: response,
                    isTested: true,
                  });
