...
class TelegramActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(TELEGRAM_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    ...
    if (
      ...
      let apiUrl =
        item.connectionData.endPoint.replace(
          "{token}",
          item.connectionData.token
        ) + item.selectedNode.selectedEvent.apiUrl;
      let methodType = item.selectedNode.selectedEvent.apiType;

      if (item.selectedNode.selectedEventName === "get_channel") {
        apiUrl = apiUrl.replace("{chat_id}", bodyInfo.username);
      }

      if (item.selectedNode.selectedEventName === "send_message") {
        apiUrl = apiUrl
          .replace("{chat_id}", bodyInfo.username)
          .replace("{text}", bodyInfo.text);
      }

      if (item.selectedNode.selectedEventName === "send_location") {
        apiUrl = apiUrl
          .replace("{chat_id}", bodyInfo.username)
          .replace("{latitude}", bodyInfo.latitude)
          .replace("{longitude}", bodyInfo.longitude);
      }

      if (item.selectedNode.selectedEventName === "set_title") {
        apiUrl = apiUrl
          .replace("{chat_id}", bodyInfo.username)
          .replace("{title}", bodyInfo.title);
      }

      if (item.selectedNode.selectedEventName === "send_contact") {
        apiUrl = apiUrl
          .replace("{chat_id}", bodyInfo.username)
          .replace("{first_name}", bodyInfo.first_name)
          .replace("{phone_number}", bodyInfo.phone);
      }

      if (item.selectedNode.selectedEventName === "send_venue") {
        apiUrl = apiUrl
          .replace("{chat_id}", bodyInfo.username)
          .replace("{latitude}", bodyInfo.latitude)
          .replace("{longitude}", bodyInfo.longitude)
          .replace("{title}", bodyInfo.title)
          .replace("{address}", bodyInfo.address);
      }

      if (item.selectedNode.selectedEventName === "send_photo") {
        apiUrl = apiUrl
          .replace("{chat_id}", bodyInfo.username)
          .replace("{photo}", bodyInfo.photo);
      }

      if (item.selectedNode.selectedEventName === "send_audio") {
        apiUrl = apiUrl
          .replace("{chat_id}", bodyInfo.username)
          .replace("{audio}", bodyInfo.audio);
      }
      ...
      try {
        httpClient
          .post(TELEGRAM_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
