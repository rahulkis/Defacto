...
class ClickUpActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  handleTestAction = () => {
    ...
    if (
      ...
      if (item.selectedNode.selectedEventName === "create_list") {
        apiUrl = apiUrl.replace("{id}", bodyInfo.workspace);
        bodyInfo = {
          "name": bodyInfo.list_name,
          "content": bodyInfo.list_info,
      ...
      if (item.selectedNode.selectedEventName === "create_folder") {
        apiUrl = apiUrl.replace("{id}", bodyInfo.workspace);
        bodyInfo = {
          "name": bodyInfo.folder_name
        }
      }

      if (item.selectedNode.selectedEventName === "new_checklist") {
        apiUrl = apiUrl.replace("{id}", bodyInfo.task);
        bodyInfo = {
          "name": bodyInfo.checklist_name
        }
      }


      if (item.selectedNode.selectedEventName === "time_tracked") {
        apiUrl = apiUrl.replace("{id}", bodyInfo.task);
        bodyInfo = {
      ...
      if (item.selectedNode.selectedEventName === "task_comment") {
        apiUrl = apiUrl.replace("{id}", bodyInfo.task);
        bodyInfo = {
          "comment_text": bodyInfo.comment,

        }
      }

      if (item.selectedNode.selectedEventName === "create_task") {
        apiUrl = apiUrl.replace("{id}", bodyInfo.list);
        bodyInfo = {
          "name": bodyInfo.task_name,
          "description": bodyInfo.task_desc,
          "priority": bodyInfo.priority,
          "due_date": bodyInfo.due_date,
      ...
      if (item.selectedNode.selectedEventName === "create_subtask") {
        apiUrl = apiUrl.replace("{id}", bodyInfo.list);
        bodyInfo = {
          "name": bodyInfo.task_name,
          "description": bodyInfo.task_desc,
      ...
      try {
        httpClient
          .post(CLICKUP_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
