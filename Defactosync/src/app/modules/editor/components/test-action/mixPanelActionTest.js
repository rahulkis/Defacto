...
import {
  APP_IMAGE_URL,
  IMAGE_FOLDER,
  MIXPANEL_WEBHOOK_URLS,
} from "constants/AppConst";

class MixPanelActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(MIXPANEL_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    ...
    if (
      ...
      if (item.selectedNode.selectedEventName === "create_update_profile") {
        bodyInfo = {
          $token: `${item.connectionData.token}`,
          $distinct_id: `${bodyInfo.distinct_id}`,
          $set: {
      ...
      if (item.selectedNode.selectedEventName === "track_event") {
        bodyInfo = {
          event: `${bodyInfo.event}`,
          properties: {
            distinct_id: `${bodyInfo.distinct_id}`,
            token: `${item.connectionData.token}`,
          },
      ...
      try {
        httpClient
          .post(MIXPANEL_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
