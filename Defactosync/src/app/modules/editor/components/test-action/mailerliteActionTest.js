...
class MailerliteActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(MAILERLITE_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {    
    ...
    if (
      ...
      if (item.selectedNode.selectedEventName === "unsubscribe_Subscriber") {
        apiUrl = apiUrl.replace("{email}", bodyInfo.email);
        bodyInfo={
          type:"unsubscribed"
        }
      }
      if (item.selectedNode.selectedEventName === "remove_Subscriber_group") {
        apiUrl = apiUrl.replace("{groupId}", bodyInfo.group_id).replace("{email}", bodyInfo.email);        
      }
      
      if (item.selectedNode.selectedEventName === "create_Subscriber") {
        apiUrl = apiUrl.replace("{groupId}", bodyInfo.group_id);
        
        bodyInfo= {
          email:bodyInfo.email,
          name:bodyInfo.name,
          resubscribe:bodyInfo.resubscribe,
          autoresponders:bodyInfo.autoresponders,
          fields:{    
      ...
      const body = {
        headerValue: {
          "X-MailerLite-ApiKey": item.connectionData.token,
      ...
      try {
        httpClient
          .post(MAILERLITE_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {
            if (res.data.statusCode === 200) {            
