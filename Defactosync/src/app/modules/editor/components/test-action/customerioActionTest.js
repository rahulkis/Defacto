...
import {
  APP_IMAGE_URL,
  IMAGE_FOLDER,
  CUSTOMERIO_WEBHOOK_URLS,
} from "constants/AppConst";

class CustomerioActionTest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: undefined,
  ...
  componentDidMount() {
    ...
    if (item.selectedNode.isConnectionTested) {
      try {
        ...
        httpClient
          .post(CUSTOMERIO_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
          .then((res) => {
  ...
  handleTestAction = () => {
    ...
    if (
      ...
      if (item.selectedNode.selectedEventName === "create-update-person") {
        apiUrl = apiUrl.replace("{identifier}", bodyInfo.id);
        bodyInfo = {
          email: bodyInfo.email,
          first_name: bodyInfo.first_name,
          Attributes: {
            [bodyInfo.data1_key]: bodyInfo.data1_value,
            [bodyInfo.data2_key]: bodyInfo.data2_value,
          },
        };

        cleanObject(bodyInfo);
        if (bodyInfo.Attributes) {
          for (let [key] of Object.entries(bodyInfo.Attributes)) {
            if (!key) {
              delete bodyInfo.Attributes[key];
            }
          }
          cleanObject(bodyInfo.Attributes);
        }

        if (
          bodyInfo.Attributes &&
          Object.keys(bodyInfo.Attributes).length === 0
        ) {
          delete bodyInfo.Attributes;
        }
      }
      if (item.selectedNode.selectedEventName === "create-event") {
        apiUrl = apiUrl.replace("{identifier}", bodyInfo.id);
        bodyInfo = {
          name: bodyInfo.event_name,
          data: {
            [bodyInfo.data1_key]: bodyInfo.data1_value,
            [bodyInfo.data2_key]: bodyInfo.data2_value,
          },
        ...
        if (bodyInfo.data) {
          for (let [key] of Object.entries(bodyInfo.data)) {
            if (!key) {
              delete bodyInfo.data[key];
            }
          }
          cleanObject(bodyInfo.data);
        }
        if (bodyInfo.data && Object.keys(bodyInfo.data).length === 0) {
          delete bodyInfo.data;
        }
      }

      if (item.selectedNode.selectedEventName === "create_anonymous_event") {
        bodyInfo = {
          name: bodyInfo.event_name,
          anonymous_id: bodyInfo.anonymous_id,
          data: {
            [bodyInfo.data1_key]: bodyInfo.data1_value,
            [bodyInfo.data2_key]: bodyInfo.data2_value,
          },
      ...
      try {
        httpClient
          .post(CUSTOMERIO_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
          .then((res) => {
            if (res.data.statusCode === 200) {
