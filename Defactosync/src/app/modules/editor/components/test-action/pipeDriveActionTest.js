...
import {
    ...
    IMAGE_FOLDER,
    SWELL_WEBHOOK_URLS,
    PIPEDRIVE_WEBHOOK_URLS
} from "constants/AppConst";


class PipeDriveActionTest extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            anchorEl: undefined,
    ...
    componentDidMount() {
        ...
        if (item.selectedNode.isConnectionTested) {
            try {
                ...
                httpClient
                    .post(PIPEDRIVE_WEBHOOK_URLS.GET_RESPONSE_BY_NODE_ID, body)
                    .then((res) => {
                        if (res.data.statusCode === 200) {
                            let response = item.selectedNode.selectedEventName === "create_deal"
                                || item.selectedNode.selectedEventName === "create_activity"
                                || item.selectedNode.selectedEventName === "create_lead"
                                || item.selectedNode.selectedEventName === "create_product"
                                || item.selectedNode.selectedEventName === "create_person"
                                || item.selectedNode.selectedEventName === "create_organization"
    ...
    handleTestAction = () => {
        ...
        if (
            ...
            if (item.selectedNode.selectedEventName === "create_deal") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    title: bodyInfo.title,
                    add_time: bodyInfo.creation_date,
                    status: bodyInfo.status,
                    stage_id: bodyInfo.stage,
                    user_id: bodyInfo.owner,
                    org_id: bodyInfo.organization,
                    person_id: bodyInfo.person,
                    expected_close_date: bodyInfo.expected_close_date,
                    value: bodyInfo.value,
                    currency: bodyInfo.currency,
                    visible_to: bodyInfo.visible_to
                };
                cleanObject(bodyInfo);
            };

            if (item.selectedNode.selectedEventName === "update_deal") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{id}", bodyInfo.deal)
                    .replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    title: bodyInfo.title,
                    status: bodyInfo.status,
                    stage_id: bodyInfo.stage,
            ...
            if (item.selectedNode.selectedEventName === "create_activity") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    subject: bodyInfo.subject,
                    org_id: bodyInfo.organization,
                    user_id: bodyInfo.assign_to,
                    person_id: bodyInfo.person,
                    deal_id: bodyInfo.deal,
                    done: bodyInfo.is_done,
                    type: bodyInfo.type,
                    due_date: bodyInfo.due_date_and_time !== undefined && bodyInfo.due_date_and_time.split(" ")[0],
                    due_time: bodyInfo.due_date_and_time !== undefined && bodyInfo.due_date_and_time.split(" ")[1],
            ...
            if (item.selectedNode.selectedEventName === "create_lead") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    title: bodyInfo.title,
                    person_id: bodyInfo.person,
                    organization_id: bodyInfo.organization,
                    owner_id: bodyInfo.owner,
                    label_ids: [bodyInfo.label],
                    expected_close_date: bodyInfo.expected_close_date,
                    visible_to: `${bodyInfo.visible_to}`
                };
                cleanObject(bodyInfo);
            ...
            if (item.selectedNode.selectedEventName === "create_product") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    name: bodyInfo.name,
                    code: bodyInfo.code,
                    unit: bodyInfo.unit,
                    tax: bodyInfo.tax_percentage,
                    active_flag: bodyInfo.is_active || 1,
                    owner_id: bodyInfo.owner,
                    visible_to: bodyInfo.visible_to,
                    prices: [
                        {
                            ...
                            currency: bodyInfo.currency,
                            cost: bodyInfo.cost,
                            overhead_cost: bodyInfo.overhead_cost
                        }
                    ]
                };
                cleanObject(bodyInfo);
            ...
            if (item.selectedNode.selectedEventName === "update_person") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{id}", bodyInfo.person)
                    .replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    name: bodyInfo.name,
                    owner_id: bodyInfo.owner,
            ...
            if (item.selectedNode.selectedEventName === "create_person") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    name: bodyInfo.name,
                    owner_id: bodyInfo.owner,
            ...
            if (item.selectedNode.selectedEventName === "update_activity") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{id}", bodyInfo.activity)
                    .replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    done: bodyInfo.is_done,
                    subject: bodyInfo.subject,
            ...
            if (item.selectedNode.selectedEventName === "create_organization") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    name: bodyInfo.name,
                    owner_id: bodyInfo.owner,
            ...
            if (item.selectedNode.selectedEventName === "update_organization") {
                apiUrl = apiUrl + item.selectedNode.selectedEvent.apiUrl.replace("{id}", bodyInfo.organization)
                .replace("{APITOKEN}", item.connectionData.token);
                bodyInfo = {
                    name: bodyInfo.name,
                    owner_id: bodyInfo.owner,
            ...
            try {
                httpClient
                    .post(PIPEDRIVE_WEBHOOK_URLS.GET_TEST_ACTION_REQUEST, body)
                    .then((res) => {
                        if (res.data.statusCode === 200) {
