...
class KeapInfusionsoftAuthForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentWillMount() {
    if (this.props.code !== "") {
      ...
      if (uri.indexOf("?") > 0) {
        ...
        this.setState({ KeapInfusionSoftCode: this.props.code });
      }
    } else {
      this.getAccountList();
    }
  ...
  getAccessToken = () => {
    if (this.props.code !== "") {
      ...
      PostData(KEAP_AUTH_URLS.ACCESS_TOKEN_URL, formModel).then((result) => {
        debugger;
        if (result != null) {
  ...
  get_accountInfo = async (res) => {
    let formdata = {
      ...
      APIUrl: KEAP_AUTH_URLS.BASE_URL + "crm/rest/v1/account/profile",
    };
    try {
      await PostData(KEAP_AUTH_URLS.GET_API, formdata).then((result) => {
        if (result.statusCode === 200) {
          this.addAuthAccnt(result.res, res);
  ...
  addAuthAccnt = (profileInfo, tokenInfo) => {
    profileInfo = JSON.parse(profileInfo);
    debugger;
    let formModel = {
      ID: DraftJS.genKey(),
      TeamName: profileInfo.name,
      Type: "KeapInfusionsoft",
      CreatedAt: Date.now(),
      RefreshToken: tokenInfo.refresh_token,
      keyType: "accessToken",
      CreatedBy: this.loginUserId,
      APIUrl: tokenInfo.scope.split('|')[1],
      Email:"",
      UserInfo: JSON.stringify(profileInfo)
    };
...
//   };
  ...
  getAccountList = () => {
    GetData(GOOGLEAUTH_URLS.GET_ACCOUNT_URL).then((result) => {
      ...
      this.setState({
        ...
        let arr = [];
        let googleArr = result.Items.filter((data) => data.Type === "KeapInfusionsoft");
  ...
  removeAccount = () => {
    ...
    if (confirm) {
      Delete(
        KEAP_AUTH_URLS.REMOVE_ACCOUNT + this.state.selectedAccntID
      ).then((result) => {
              ...
              <div className="FieldConfigurationField ">
                ...
                <div className="FieldConfiguration__value">
                  ...
                  <div style={{ paddingTop: "18px" }}>
                    ...
                    <a
                      href={KEAP_AUTH_URLS.AUTH_URL.replace(
                        "{CLIENT_ID}",
...
export default KeapInfusionsoftAuthForm;
