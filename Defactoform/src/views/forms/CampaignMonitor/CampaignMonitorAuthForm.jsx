...
import {
  getCampaignMonitorClientList,
  getCampaignMonitorSubscriberList,
  AddUpdateCampaignMonitor,
  Add_Asana_Task_ToSection,
} from "../../../API/IntegrationAPI";
import { calculateTime } from "../../../util/commonFunction";

class CampaignMonitorAuthForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      selectedAccntID: "",
      headerName: "Add or Update a Subscriber",
  ...
  getAccessToken = () => {
    if (this.props.code !== "") {
      ...
      bodyFormData.append("client_id", CAMPAIGN_MONITOR_AUTH_URLS.CLIENT_ID);
      bodyFormData.append("client_secret", CAMPAIGN_MONITOR_AUTH_URLS.CLIENT_SECRET);
      ...
      PostDataIntegation(CAMPAIGN_MONITOR_AUTH_URLS.ACCESS_TOKEN_URL, bodyFormData).then(
        (result) => {
          if (result != null) {
            if (result.access_token !== undefined) {
              this.setItem({access_Token : result.access_token, refresh_Token : result.refresh_token});
  ...
  getAccessTokenFromRefershToken = async (token) => {
    ...
    await PostDataIntegation(CAMPAIGN_MONITOR_AUTH_URLS.ACCESS_TOKEN_URL, bodyFormData).then(
      (result) => {
        if (result != null) {
          if (result.access_token !== undefined) {
            this.setState({access_Token:result.access_token});
            this.BindClientList(result.access_token);
          }
        }
      }
    );
  ...
  getAccountList = async () => {
    GetData(GOOGLEAUTH_URLS.GET_ACCOUNT_URL).then((result) => {
      if (result != null) {
        let arr = [];
        let googleArr = result.Items.filter((data) => data.Type === "CampaignMonitor");
  ...
  BindClientList = async (token) => {
    let clientlist = await getCampaignMonitorClientList(
      CAMPAIGN_MONITOR_AUTH_URLS.BASE_URL,
      token
    );
    if(clientlist.length > 0)
    {
        this.setState({ clientlists: clientlist });
    }
  };
  ...
  handleClientChange = async (e) => {
    this.setState({ clientId: e.value });
     let _self= this;
     var token = _self.state.access_Token;
     let subscriberResult = await getCampaignMonitorSubscriberList(CAMPAIGN_MONITOR_AUTH_URLS.BASE_URL, token,e.value);
     this.setState({ subscriberList: subscriberResult });
  };

  handelSubcriberListChange =async (e) =>
  {
    this.setState({ listId: e.value });
    let _self= this;
    var token = _self.state.access_Token;
    this.getCustomeFieldsForSelectedlist(e.value,token);
  }
  ...
  removeAccount = () => {
    ...
    if (confirm) {
      Delete(CAMPAIGN_MONITOR_AUTH_URLS.REMOVE_ACCOUNT + this.state.selectedAccntID).then(
        (result) => {
  ...
  getCustomeFieldsForSelectedlist(listId,token) {
    this.setState({
      isLoadingFields: true,
    });
    let formModel = {
      APIUrl: CAMPAIGN_MONITOR_AUTH_URLS.BASE_URL +  "api/v3.2/lists/"+ listId +"/customfields.json",
      headerValue:
        "Bearer " + token,
    };
    try {
      PostData(CAMPAIGN_MONITOR_AUTH_URLS.GET_API, formModel).then(
        (result) => {
  ...
  handlerChangeFieldData(key, event) {
    ...
    const newData = {Key:key,Value:event.target.value};
    const objIndex = this.state.fieldsDataToSubmit.findIndex((obj) => obj.Key == key)
  ...
  sendTest = async () => {
    ...
    if (!this.state.isValidated) {
      ...
      let result = {};
      try {
        await this.BindWorkClientWitRefershToken(
        ...
        result = await AddUpdateCampaignMonitor(
          token,
          _self.state.listId,
          dataObj
        );
  ...
  finishSetUp = () => {
    let campaignMonitorSetUpData = {};

    campaignMonitorSetUpData = {
    ...
    let formModel = {
      ...
      IntegrationType:"CampaignMonitor",
      FormId: localStorage.CurrentFormId,
      SetUpData: JSON.stringify(campaignMonitorSetUpData),
      CreatedAt: Date.now(),
      CreatedBy: this.loginUserId,
              ...
              <div className="FieldConfigurationField ">
                <div className="FieldConfiguration__label">
                  ...
                  <div className="FieldConfigurationField__i">
                    ...
                    <div className="FieldConfigurationField__description">
                      ...
                      You can connect a new Campaign Monitor account, or choose from the list of previously connected accounts.
                      </div>
                    </div>
                  </div>
                </div>
                <div className="FieldConfiguration__value">
                  ...
                  <div style={{ paddingTop: "18px" }}>
                    ...
                    <a
                      href={CAMPAIGN_MONITOR_AUTH_URLS.AUTH_URL.replace(
                        "{CLIENT_ID}",
                        CAMPAIGN_MONITOR_AUTH_URLS.CLIENT_ID
                      ).replace(
                        "{REDIRECT_URI}",
                        `${window.location.origin.toString()}/user/IntegrationNwebhooks`
                      )}
                  ...
                  <div>
                    <div className="FieldConfigurationField ">
                      ...
                      <div className="FieldConfiguration__value">
                        <Select
                          isClearable={true}
                          options={this.state.clientlists}
                      ...
                      <div className="FieldConfigurationField">
                        ...
                        <div className="FieldConfiguration__value">
                          <Select
                            ...
                            onChange={(e) => this.handelSubcriberListChange(e)}
                          />
                          {this.state.listId === "" && (
                            <div className="FieldConfigurationField__error">
                              This field is required
                          ...
                          >
                            <div className="FieldConfigurationField">
                              <h4>Loading...</h4>
                                          ...
                                          {!this.state.isLoadingFields && this.state.fieldsList.map(
                                            (field, index) => {
                                              return (
                                                <div
...
export default CampaignMonitorAuthForm;
